...
function updateSection(scope, markup) {
  ...
  for (var index = 0; index < length; index++) { 
    ...
    if (/^(struct|section|expression)/.test(field.type) && !field.param) {
 ...
 *
 *  There is 11 different types of fields, each with it's own set of
 *  properties. 
 *
 ...
 *  `object`, `regexp`, `expression`, `path`, `static`, `section` and
 *  `struct.
 *
 *  ### boolean
 *
